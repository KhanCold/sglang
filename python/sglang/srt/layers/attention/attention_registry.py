...
def attn_backend_wrapper(runner: "ModelRunner", full_attn_backend: "AttentionBackend"):
    ...
    """
    assert not (
        hybrid_gdn_config(model_runner_ref=runner) is not None
        and runner.use_mla_backend
    ), "hybrid_gdn can only be used with non-MLA models."

    if cfg := mambaish_config(model_runner_ref=runner):
        ...
        if hybrid_gdn_config(model_runner_ref=runner) is not None:
            if is_blackwell():
                assert (
        ...
        elif mamba2_config(model_runner_ref=runner) is not None:
            linear_attn_backend = Mamba2AttnBackend(runner)
        elif kimi_linear_config(model_runner_ref=runner) is not None:
            linear_attn_backend = KDAAttnBackend(runner)
        elif hybrid_lightning_config(model_runner_ref=runner) is not None:
