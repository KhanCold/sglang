...
class Scheduler(
    ...
    def init_cache_with_memory_pool(self):
        ...
        _spec = linear_attn_model_spec(model_runner_ref=self.tp_worker.model_runner)
        _registry_needs_mamba = (
            _spec.uses_mamba_radix_cache if _spec is not None else False
        )
        self.is_hybrid_ssm = (
            hybrid_gdn_config(model_runner_ref=self.tp_worker.model_runner) is not None
            or mamba2_config(model_runner_ref=self.tp_worker.model_runner) is not None
