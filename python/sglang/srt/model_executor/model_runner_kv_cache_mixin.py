...
class ModelRunnerKVCacheMixin:
    def _profile_available_bytes(self: ModelRunner, pre_model_load_memory: int) -> int:
        ...
        if mambaish_config(model_runner_ref=self) is not None:
            rest_memory = self.handle_max_mamba_cache(rest_memory)

        return int(rest_memory * (1 << 30))  # return in bytes

    def handle_max_mamba_cache(self: ModelRunner, total_rest_memory):
        config = mambaish_config(model_runner_ref=self)
    ...
    def _init_pools(self: ModelRunner):
        ...
        if self.req_to_token_pool is None:
            ...
            if self.server_args.disaggregation_mode == "decode":
                ...
                if config := mambaish_config(model_runner_ref=self):
            ...
            elif config := mambaish_config(model_runner_ref=self):
        ...
        elif self.use_mla_backend and not mambaish_config(model_runner_ref=self):
        ...
        if self.token_to_kv_pool_allocator is None:
            if current_platform.is_out_of_tree():
                ...
                or hybrid_gdn_config(model_runner_ref=self) is not None
            ):
                if self.is_hybrid_swa:
